...
- **That same fallback chain now also tries Webull's own order-history
  endpoint before ever reaching a live snapshot, at the user's explicit
  request ("use the webull api history if you can instead of a
  snapshot").** A genuine historical fill price is strictly better
  evidence than a live quote taken at detection time -- which could be
  minutes and a real price move away from the actual closing fill. New
  `WebullBrokerClient.get_order_history(start_date=None, end_date=None)`
  wraps `order_v3.get_order_history` (NORMAL priority) and a new
  `TradingLoop._latest_filled_price_from_history(rows, symbol, exit_side)`
  scans its raw rows for the most recent `FILLED` row matching the
  symbol and exit side, trying several plausible fill-price field names
  since none has been confirmed against a real populated response yet.
  Wired in immediately after `poll_fills` and ahead of
  `stop_price`/`target_price`/the live snapshot above, via
  `getattr(self.broker, "get_order_history", None)` so brokers that
  don't implement it (every test double, currently) skip the branch
  entirely. **This layers on top of, not instead of, the snapshot fix
  above** -- as documented in `docs/ARCHITECTURE.md`, a live query
  against this sandbox account has come back an empty list despite 70+
  known real orders, for reasons still not understood, so this lookup is
  expected to frequently contribute nothing until that's resolved; the
  snapshot and entry-price fallbacks below it are what actually keep
  firing in the meantime. See
  `tests/test_trading_loop.py::test_reconcile_external_close_prefers_order_history_over_a_snapshot`,
  `::test_reconcile_external_close_falls_through_when_history_has_no_match`,
  `::test_latest_filled_price_from_history_picks_the_most_recent_match`,
  and `::test_latest_filled_price_from_history_returns_none_when_nothing_matches`.
- **Webull's own OpenAPI spec for `get_order_detail` (supplied by the
  user) caught two real bugs in code that had been written against
  unconfirmed guesses.** `get_order_status` assumed the response was
  either a bare list or a single order dict; the documented shape is
  always `{"orders": [...]}`, and a combo/bracket's `orders` array can
  hold multiple legs -- the old code could have silently parsed the wrong
  leg. Now unwraps `orders` and picks the row whose `client_order_id`
  matches what was actually asked for. Separately, `_order_from_detail`
  was reading a `quantity` key that's actually `total_quantity` per the
  spec (matching `get_order_open`'s independently live-confirmed shape) --
  `quantity` is now only a fallback. The spec also confirms `filled_price`
  as a real field name, which `_latest_filled_price_from_history` (see
  below) now tries first instead of guessing. See
  `docs/ARCHITECTURE.md`'s "Webull integration" section for the full
  writeup and `tests/test_webull_broker_client.py`'s new
  `test_get_order_status_picks_the_matching_leg_out_of_a_combo_response`
  and `test_order_from_detail_prefers_total_quantity_over_quantity`.
