...
def test_attach_broker_bracket_gives_up_after_max_consecutive_failures():
    # Real incident, 2026-08-12: two positions whose broker-side data was
    # permanently inconsistent (a resting sell rejected as "would reverse
    # the position" regardless of order type/session/quantity) each burned
    # a real place_order call, at CRITICAL rate-limiter priority, on EVERY
    # tick forever -- starving candidate discovery. This must stop
    # retrying once TradingLoopConfig.max_broker_bracket_attach_failures
    # (default 5) is reached.
    @dataclass
    class _PermanentlyBrokenBroker(_RestingBroker):
        place_order_calls: int = 0

        def place_oco_bracket(self, stop_order, target_order):
            self.place_order_calls += 1
            raise RuntimeError("OAUTH_OPENAPI_ORDER_NOT_SUPPORT_REVERSE_OPTION")

    broker = _PermanentlyBrokenBroker()
    loop, candidate = _armed_candidate_setup(broker)
    # target_price set (not None) so this exercises the same
    # place_resting_bracket/place_oco_bracket path the real incident hit
    # -- target_price=None would take the lone-stop branch instead
    # (place_resting_stop -> plain place_order).
    position = Position(
        symbol="TEST", side=OrderSide.BUY, quantity=10, avg_entry_price=5.00,
        stop_price=4.50, target_price=6.00, trailing_stop_pct=None,
        opened_at=datetime.utcnow(), strategy_name="test",
    )

    for _ in range(loop.config.max_broker_bracket_attach_failures):
        loop._attach_broker_bracket(candidate, position, _IN_HOURS_NOW)
    assert broker.place_order_calls == loop.config.max_broker_bracket_attach_failures
    assert position.broker_bracket_attach_failures == loop.config.max_broker_bracket_attach_failures

    # One more tick -- no-ops before ever calling the broker again.
    loop._attach_broker_bracket(candidate, position, _IN_HOURS_NOW)
    assert broker.place_order_calls == loop.config.max_broker_bracket_attach_failures
    assert position.broker_stop_order_id is None  # still riding on software-only management


def test_attach_broker_bracket_resets_the_failure_counter_on_success():
    @dataclass
    class _FlakyThenWorkingBroker(_RestingBroker):
        fail_times: int = 0
        ...
        def place_oco_bracket(self, stop_order, target_order):
            if self.fail_times > 0:
                self.fail_times -= 1
                raise RuntimeError("simulated transient placement failure")
            return super().place_oco_bracket(stop_order, target_order)

    broker = _FlakyThenWorkingBroker(fail_times=2)
    loop, candidate = _armed_candidate_setup(broker)
    position = Position(
        symbol="TEST", side=OrderSide.BUY, quantity=10, avg_entry_price=5.00,
        stop_price=4.50, target_price=6.00, trailing_stop_pct=None,
    ...
    loop._attach_broker_bracket(candidate, position, _IN_HOURS_NOW)  # fails
    loop._attach_broker_bracket(candidate, position, _IN_HOURS_NOW)  # fails
    assert position.broker_bracket_attach_failures == 2
    loop._attach_broker_bracket(candidate, position, _IN_HOURS_NOW)  # succeeds
    assert position.broker_bracket_attach_failures == 0
    assert position.broker_stop_order_id is not None
