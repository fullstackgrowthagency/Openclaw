...
exit Signal should be generated (stop, target, trailing stop, VWAP failure,
time limit). Momentum-based exits are left as a hook for strategy-specific
logic since "momentum failed" is strategy-dependent.
...
class PositionManager:
    ...
    def check_exit(self, position: Position, snapshot: MarketSnapshot, *, now: Optional[datetime] = None) -> Optional[Signal]:
        ...
        elif position.target_price is not None and snapshot.last_price >= position.target_price:
