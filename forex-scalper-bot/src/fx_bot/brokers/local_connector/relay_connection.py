...
`recv() -> str`, `close()` -- a real `websockets` connection already
does, with no wrapper needed; a future FastAPI `WebSocket` adapter would
just need to expose the same three methods.
"""
...
class RelayConnection:
    ...
    def _handle_frame(self, raw: str) -> None:
        ...
        # REQUEST/AUTH frames arriving here are ignored in Phase 5b -- no
        # pairing/auth handshake exists yet (that's Phase 5c).
